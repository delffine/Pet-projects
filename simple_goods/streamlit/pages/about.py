...
with mainblok:
    ...
    ***Примечания**:
    - Подписчики определяются по наличию транзакций с индификатором подписки.
    - Покупатели определяются по наличию транзакций с типом операции "Оплата". (При предобработке переименовывается в "Покупка").
    - В когортном анализе не учитываются плательщики, которые сделали разовые транзакции более 25 тысяч рублей, так как они существенно искажают статистику своей когорты.
    - В когортном анализе месяц не календарный - а период в 30 дней, отсчет идет от максимальной даты среди всех транзакции.
